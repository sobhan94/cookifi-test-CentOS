...
class Large_Order(unittest.TestCase):
    def setUp(self):
...
#        self.google_login()
    ...
    def alternate_login(self):
        driver = self.driver
        email = 'cookifi.test@gmail.com'
        password = 'N2-HsIYd0Go'
        driver.find_element_by_partial_link_text("By password").click()
        driver.find_element_by_id('id_username').send_keys(email)
        driver.find_element_by_id('id_password').send_keys(password)
        form = driver.find_element_by_xpath("//form[@method='post']")
        form.submit()
    ...
    def fill_menu(self):
        ...
        self.add_dish(item)
        self.check_price(str('21,700.00'), item)
        item = 'Chicken Kebab'
        self.add_dish(item)
        self.check_price(str('21,700.00'), item)
        ...
        self.add_dish(item)
        self.check_price(str('24,700.00'), item)
        # cutlery set
        item = 'Disposable Set'
        self.add_dish(item)
        self.check_price(str('24,700.00'), item)
    ...
    def check_price(self, cost, item):
        ...
        element = driver.find_element_by_xpath("//td[contains(text(), '₹"+ cost +"')]")
